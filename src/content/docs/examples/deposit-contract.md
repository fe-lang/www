...
description: A shape-for-shape Fe port of the Ethereum 2.0 deposit contract
---
...
The contract's public ABI is declared with a `msg` block. Each variant carries an explicit
...
### Storage and the Merkle Branch

State lives in a `DepositStore` storage struct holding the incremental Merkle `branch`, the
...
The `Deposit` handler is marked `#[payable]` because it receives ETH. It mirrors the Solidity
...
`GetDepositRoot` walks the branch combining it with the zero-hash table, then mixes in the deposit
...
`SupportsInterface` implements ERC-165 interface detection, returning `true` for the ERC-165
